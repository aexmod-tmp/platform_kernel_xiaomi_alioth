...
static int get_step_chg_jeita_setting_from_profile(struct step_chg_info *chip)
{
	...
	rc = of_property_read_u32(profile_node, "qcom,jeita-too-hot",
					&chip->jeita_hot_th);
	if (rc < 0) {
		pr_debug("do not use external fg and set jeita to hot to invaled\n");
	...
	rc = of_property_read_u32(profile_node, "qcom,jeita-too-cold",
					&chip->jeita_cold_th);
	if (rc < 0) {
		pr_debug("do not use external fg and set jeita too cold to invaled\n");
	...
	rc = of_property_read_u32(profile_node, "qcom,jeita-warm-th",
					&chip->jeita_warm_th);
	if (rc < 0) {
		pr_debug("do not use dtsi config and set jeita warm to invaled\n");
	}
	...
	rc = of_property_read_u32(profile_node, "qcom,jeita-cool-th",
					&chip->jeita_cool_th);
	if (rc < 0) {
		pr_debug("do not use dtsi config and set jeita cool to invaled\n");
	}
	chip->use_bq_pump =
...
static int handle_fast_charge(struct step_chg_info *chip, int temp)
{
	...
	if (is_dc_wls_available(chip)) {
		...
		if (rc < 0)
			pr_debug("Couldn't get cp_en rc = %d\n", rc);
		else
...
static int handle_jeita(struct step_chg_info *chip)
{
	...
	pr_debug("battery warm = %d battery cool = %d\n", chip->jeita_warm_th, chip->jeita_cool_th);
